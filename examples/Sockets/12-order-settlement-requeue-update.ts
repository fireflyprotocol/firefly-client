...
async function main() {
  ...
  const settlementUpdateCallback = (
    update: OrderSentForSettlementUpdateResponse
  ) => {
    console.log(update);
  };

  const requeueUpdateCallback = (update: OrderRequeueUpdateResponse) => {
    console.log(update);
  };
  ...
  const connection_callback = async () => {
    ...
    client.sockets.onUserOrderSentForSettlementUpdate(settlementUpdateCallback);
    // triggered when order is requeued
    client.sockets.onUserOrderRequeueUpdate(requeueUpdateCallback);
  };
