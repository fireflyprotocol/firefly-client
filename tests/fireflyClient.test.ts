...
import { toBigNumberStr, Networks, MARKET_SYMBOLS, ORDER_STATUS, 
    FireflyClient, bnStrToBaseNumber, ORDER_SIDE } from "../src/index";
...
describe("FireflyClient", () => {
    ...
    it("should return public address of account", async ()=>{
        const client = new FireflyClient(Networks.TESTNET, testAcctKey);
        expect(client.getPublicAddress()).to.be.equal("0xe83515fEa858D4ac48278F27DF375fbF2bff441d");
    });
    ...
    describe("Get Orders", () => {
        let client = new FireflyClient(Networks.TESTNET, testAcctKey)
        ...
        it("should get all cancelled orders", async ()=>{
            const data = await client.getOrders({status: ORDER_STATUS.CANCELLED, symbol:MARKET_SYMBOLS.DOT});
            expect(data.length).to.be.greaterThanOrEqual(1)
        });

        it("should get 1 cancelled orders", async ()=>{
            const data = await client.getOrders({status: ORDER_STATUS.CANCELLED, symbol:MARKET_SYMBOLS.DOT, pageSize:1});
            expect(data.length).to.be.equals(1)
        });

        it("should get 0 expired orders as page 2 does not exist for expired orders", async ()=>{
            const data = await client.getOrders({status: ORDER_STATUS.EXPIRED, symbol:MARKET_SYMBOLS.DOT, pageNumber:2});
            expect(data.length).to.be.equals(0)
        });

    });

    describe.only("Create/Post Orders", () => {
        let client = new FireflyClient(Networks.TESTNET, testAcctKey)
        ...
        it("should throw error as DOT market is not added to client", async ()=>{
            await expect(client.createSignedOrder({
        ...
        it.only("should place a LIMIT order on exchange", async ()=>{
