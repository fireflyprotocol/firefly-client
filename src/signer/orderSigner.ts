...
import {
  ...
  EIP712_CANCEL_ORDER_STRUCT, DEFAULT_EIP712_DOMAIN_NAME, EIP712_CANCEL_ORDER_STRUCT_STRING,
  EIP712_ORDER_STRUCT_STRING, EIP712_ORDER_SIGNATURE_TYPE_ARR, EIP712_DOMAIN_STRING,
...
export class OrderSigner {
  ...
  ): [balance: Balance, positionMargin: BigNumber, walletBalance: BigNumber, error?: ValidationError] {
  ...
  public static calculateFreedCollateralByTrade(
    ...
    if (accountBalance.size.eq(0) || accountBalance.size.isPositive() === isBuy) {
      return {
        flags: 1,
    ...
    if (!accountBalance.size.eq(0)) {
      ...
      openInterest = accountBalance.size.isPositive()
        ? accountBalance.debt.isPositive()
          ? positionMargin.minus(accountBalance.debt.abs().times(BIGNUMBER_BASE))
          : positionMargin.abs().plus(accountBalance.debt.abs().times(BIGNUMBER_BASE))
    ...
    if (
      ...
      const pnlIsPositive = bookedPNL.isPositive() !== accountBalance.size.isPositive();
      if (!pnlIsPositive) {
  ...
  public static getUpdatedBalanceAndPositionMargin(
    ...
    if (flags & 2) {
      ...
      if (collateral.eq(0)
                ...
                || flags & 8) {
        if (pnl.gte(fee)) {
          walletBalance = walletBalance.plus(pnl.minus(fee).div(BIGNUMBER_BASE));
          fee = new BigNumber(0);
        } else {
          fee = fee.minus(pnl);
        }
    ...
    if (flags & 1) {
      ...
      walletBalance = walletBalance.plus(collateral.minus(fee).div(BIGNUMBER_BASE));
      fee = new BigNumber(0);
    } else {
      positionMargin = positionMargin.minus(fee);
      fee = fee.minus(collateral);
      newBalance.debt = newBalance.debt.minus(collateral.plus(fee).div(BIGNUMBER_BASE));
    }
  ...
  public static getBalanceUpdatesAfterFillingOrder(
    ...
    const debtAmount = sizeAmount.times(debtPerPosition.value).dp(0, BigNumber.ROUND_DOWN);
    return {
  ...
  /**
     ...
     */
  public solidityOrderHasValidSignature(
    order: SignedSolidityOrder,
  ): boolean {
    return hashHasValidSignature(
  ...
  /**
     ...
     */
  public orderHashToCancelOrdersHash(
    orderHash: string[],
  ): string {
    ...
    const structHash = Web3.utils.soliditySha3(
      { t: 'bytes32', v: hashString(EIP712_CANCEL_ORDER_STRUCT_STRING) || '' },
      { t: 'bytes32', v: hashString('Cancel Orders') || '' },
      { t: 'bytes32', v: Web3.utils.soliditySha3(...orderHash.map((hash) => ({ t: 'bytes32', v: hash }))) || '' },
    );
    return structHash ? getEIP712Hash(this.getDomainHash(), structHash) : '';
  }
  ...
  public fillToTradeData(
    ...
    return combineHexStrings(orderAData, orderBData, fillData, signatureDataA, signatureDataB);
  }
  ...
  public fillSolidityByteOrderToTradeData(
    ...
    return combineHexStrings(orderA, orderB, fillData, signatureDataA, signatureDataB);
  }
  ...
  public tradeDataReverse(data: string): {
    ...
    return {
      orderA: {
        ...
        limitFee: new Fee(new BigNumber(res[4]).times(1e-18).times(orderAFlags.isNegativeLimitFee ? -1 : 1)),
      ...
      orderB: {
        ...
        limitFee: new Fee(new BigNumber(res[13]).times(1e-18).times(orderAFlags.isNegativeLimitFee ? -1 : 1)),
  ...
  public rawOrdertoSolidityByteOrder(
    order: RawOrder,
  ): any {
    return this.web3.eth.abi.encodeParameters(
      EIP712_ORDER_SIGNATURE_TYPE_ARR,
  ...
  public static getOrderFlags(
    order: Order | SolidityOrder | RawOrder,
  ): string {
    const booleanFlag = 0
            + (new BigNumber(order.limitFee.toString()).isNegative() ? ORDER_FLAGS.IS_NEGATIVE_LIMIT_FEE : 0)
